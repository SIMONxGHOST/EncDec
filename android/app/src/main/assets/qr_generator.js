...
(function(root, factory) {
    ...
    function generate(data) {
        ...
        return {
            ...
            toSVG: function(cellSize, margin) {
                cellSize = cellSize || 6;
                margin = margin !== undefined ? margin : 4;
                var fullSize = (size + margin * 2) * cellSize;
                var svg = ['<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + fullSize + ' ' + fullSize + '" width="' + fullSize + '" height="' + fullSize + '">'];
                ...
                for (var r = 0; r < size; r++) {
                    for (var c = 0; c < size; c++) {
                        if (mat[r][c] === 1) {
                            ...
                            pathData.push('M' + x + ',' + y + 'h' + cellSize + 'v' + cellSize + 'h-' + cellSize + 'z');
                        }
                    }
                }
                svg.push('<path d="' + pathData.join('') + '" fill="#000000"/>');
                svg.push('</svg>');
                return svg.join('');
            },
            toCanvas: function(canvas, cellSize, margin) {
                cellSize = cellSize || 6;
                margin = margin !== undefined ? margin : 4;
                var fullSize = (size + margin * 2) * cellSize;
    ...
    function drawToCanvas(text, canvas, options) {
        options = options || {};
