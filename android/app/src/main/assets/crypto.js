/**
 * EncDec Pro Crypto Engine v5.0 (JavaScript / Web Crypto API)
 * =========================================================================
...
(function(root, factory) {
    ...
    async function encryptFileBytes(fileBytes, password, keyType = "Key 4", progressCb = null, keyfileBytes = null, filename = "file.dat") {
        if (progressCb) progressCb(15, "Initializing memory-hard key derivation...");
        const config = getKeyConfig(keyType);
        const cryptoInstance = getCrypto();
        const salt = cryptoInstance.getRandomValues(new Uint8Array(config.saltLength));
        const derivedKey = await deriveMasterKey(password, salt, config, keyfileBytes, 32);

        if (progressCb) progressCb(45, "Encrypting file data with AES-256-GCM AEAD...");
        const { nonce, ciphertext } = await encryptAesGcm(fileBytes, derivedKey);

        if (progressCb) progressCb(80, "Assembling quantum-resistant ENC5F container...");
        const metadata = {
            v: 2,
            kdf: "scrypt",
            cipher: "AES-256-GCM",
        ...
        if (progressCb) progressCb(100, "File encryption complete!");
        return out;
    }

    async function decryptFileBytes(fileBytes, password, fallbackKey = "Key 4", progressCb = null, keyfileBytes = null) {
        if (progressCb) progressCb(10, "Inspecting container header...");
        ...
        if (isV5) {
            ...
            if (progressCb) progressCb(40, `Computing memory-hard key (${config.alias})...`);
            const derivedKey = await deriveMasterKey(password, salt, config, keyfileBytes, 32);

            if (progressCb) progressCb(75, "Authenticating AEAD payload...");
            const decryptedBytes = await decryptAesGcm(cipherBytes, derivedKey, nonce);

            if (progressCb) progressCb(100, "File decryption complete!");
            return {
                success: true,
            ...
            if (progressCb) progressCb(45, `Deriving legacy key for ${alias}...`);
            const legacyKey = await pbkdf2Sha256(new TextEncoder().encode(password), salt, iters, 32);

            if (progressCb) progressCb(75, "Verifying HMAC signature...");
